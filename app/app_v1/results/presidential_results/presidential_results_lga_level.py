...
from app.app_v1.results.presidential_results.partytable import presidential_table_lga
import json
...
#             FROM lga_result_table lrt),
# st as
# 		(SELECT country_id,country_name,lgat.state_id, lgat.state_name, 
# 			sum(lgat.A) AS A, sum(lgat.AA) AS AA, sum(lgat.AAC) AS AAC, 
...
def get_lga_lga_all_results(country_name="undefined",state_name="undefined", lga_name="undefined",party_data={}):
    with get_db2() as conn:
        ...
        lga_result = f"""{presidential_table_lga['query']} select  lga_name, {parties},  Total_Registered_voters,  Total_Accredited_voters, Total_Rejected_votes from lgat where 1=1 {state_query} {lga_query} """
...
def get_lga_state_all_results(country_name="undefined",state_name="undefined",party_data={}):
     with get_db2() as conn:
        ...
        state_result = f"""{presidential_table_lga['query']} select state_name,  {parties},  Total_Registered_voters,  Total_Accredited_voters, Total_Rejected_votes from st where 1=1 {state_query} """
...
def get_lga_country_all_results(country_name,party_data={}):
     with get_db2() as conn:
        ...
        country_result = f"""{presidential_table_lga['query']} select country_name,  {parties},  Total_Registered_voters,  Total_Accredited_voters, Total_Rejected_votes from ct where 1=1 """
        key_values = []
        execute_queries = []
        if country_name:
        ...
        map1 = ['COUNTRY_NAME','STATE_NAME','LGA_NAME']
        map2 = ['TOTAL_REGISTERED_VOTERS','TOTAL_ACCREDITED_VOTERS','TOTAL_REJECTED_VOTES']
        map3 = ['REMARKS']
        result = ['result']
        mapres = ['COUNTRY_NAME']
