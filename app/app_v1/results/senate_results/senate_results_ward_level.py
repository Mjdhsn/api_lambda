...
from app.app_v1.results.senate_results.partytable import presidential_table_ward
import json
...
conditions_state = {
    "total": f"""{presidential_table_ward['query']} SELECT COUNT(*) as  count1 from wt""",
    "total_registered_votes_table": f"""{presidential_table_ward['query']} select state_name,district_name,lga_name,ward_name,  Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks  from wt""",
    "total_registered_votes": f"""{presidential_table_ward['query']} SELECT COALESCE(sum(Total_Registered_voters),0) as  count1 from wt""",
    'canceled': f"""{presidential_table_ward['query']} SELECT count(*) as  count1 from wt where status ='canceled'""",  
    "canceled_table": f"""{presidential_table_ward['query']} SELECT state_name,district_name,lga_name, ward_name, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks from wt where status ='canceled' """,
    "total_registered_canceled_votes": f"""{presidential_table_ward['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 from wt where status ='canceled' """ ,   
    "collated": f"""{presidential_table_ward['query']}  SELECT COUNT(*) as  count1 from wt where   (status = 'collated' OR status='canceled')""",
    "collated_table": f"""{presidential_table_ward['query']} SELECT state_name,district_name,lga_name, ward_name,  Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks  from wt where  (status = 'collated' OR status='canceled')""",
    "total_registered_collated_votes": f"""{presidential_table_ward['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 from wt where (status = 'collated' OR status='canceled')""",
    "un_collated": f"""{presidential_table_ward['query']} SELECT COUNT(*) as  count1   from wt where status='non collated'""",
    "un_collated_table":f"""{presidential_table_ward['query']} SELECT state_name,district_name,lga_name,ward_name,   Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks from wt where status='non collated'""",
    "total_registered_uncollated_votes": f"""{presidential_table_ward['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 from wt where status='non collated'""",
...
def get_ward_ward_all_results(country_name="undefined",state_name="undefined", district_name="undefined",lga_name="undefined", ward_name="undefined",party_data={}):
    with get_db2() as conn:
        ...
        ward_result = f"""{presidential_table_ward['query']} select ward_name,  {parties} , Total_Registered_voters,  Total_Accredited_voters, Total_Rejected_votes from wt where 1=1 {state_query}  {district_query}  {lga_query} {ward_query} """
...
def get_ward_lga_all_results(country_name="undefined",state_name="undefined",district_name="undefined", lga_name="undefined",party_data={}):
    with get_db2() as conn:
        ...
        lga_result = f"""{presidential_table_ward['query']} select  lga_name, {parties},  Total_Registered_voters,  Total_Accredited_voters, Total_Rejected_votes from lgat where 1=1 {state_query}  {district_query}  {lga_query} """
...
def get_ward_state_all_results(country_name="undefined",state_name="undefined",district_name="undefined",party_data={}):
    with get_db2() as conn:
        ...
        state_result = f"""{presidential_table_ward['query']} select state_name,  {parties},  Total_Registered_voters,  Total_Accredited_voters, Total_Rejected_votes from st where 1=1 {state_query}  {district_query} """
