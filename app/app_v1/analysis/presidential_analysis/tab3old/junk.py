...
"non_collated_ward":f"""{values['pu']},  non_collated_ward as
...
"non_collated_lga":f"""{values['pu']}, non_collated_lga as
...
"non_collated_state": f"""{values['pu']}, non_collated_state as 
...
"collated_ward": f"""{values['pu']}, collated_ward as
...
( select count(*) as total,state_id, lga_id,state_name, lga_name,sum(Total_Registered_voters) as Total_Registered_voters, 
			...
			from pu  group by pu.state_id,pu.lga_id)

""",

"collated_lga": f"""{values['pu']},collated_lga as
( select count(*) as total,state_id, lga_id,state_name, lga_name,sum(Total_Registered_voters) as Total_Registered_voters, 
			(count(*) - sum(status='collated'or status='canceled')) as diff 
			from pu  group by pu.state_id,pu.lga_id)
...
win_tab2 = {
"win_pu": f"""{values_win['win']} , win_pu as
...
"win_ward":f"""{values_win['win_w']}, win_ward as
...
"win_lga":f"""{values_win['win_l']}, win_lga as
...
"win_state":f"""{values_win['win_s']} , win_state as
...
"win_country":f"""{values_win['win_c']}, win_country as
...
"state_25_above":f"""{values_win['win_s']}, state_25_above as
...
diff = {
"diff_bw_1st_2nd":
f"""{win_tab2['state_25_above']}, diff_bw_1st_2nd as
...
exp_winner = {
 ...
 "exp_winner":   
f"""{diff['diff_bw_1st_2nd']}, exp_winner as
